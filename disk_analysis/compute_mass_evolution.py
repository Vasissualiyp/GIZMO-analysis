...
guac_src_path = os.path.join(
    BASE.replace("/scratch/vasissua", "/home/vasissua").replace(
        "/home/vasilii/research/trillium/scratch",
        "/home/vasilii/research/trillium/home",
    ),
    "PYTHON/GUAC/src/",
)
pfp_src_path = os.path.join(
    BASE.replace("/scratch/vasissua", "/home/vasissua").replace(
        "/home/vasilii/research/trillium/scratch",
        "/home/vasilii/research/trillium/home",
    ),
...
def main():
    ...
    for idx, (snap_path, snap_num) in enumerate(snap_items):
        ...
        # ── Center: most massive sink, else densest gas particle ──
        # (Same logic as notebooks.make_disk_movie_frames.find_center.)
        # CRITICAL: using the gas median pre-formation is wrong — the median is
        # ~5,000–28,000 AU from the proto-disk density peak, so all disk gas is
        # missed and M_enc jumps from ~0 to ~12 Msun at sink formation.
        if (
            stardata is not None
            and "Masses" in stardata
            and len(stardata["Masses"]) > 0
        ):
            center = stardata["Coordinates"][np.argmax(stardata["Masses"])]
        elif "Density" in pdata and len(pdata["Density"]) > 0:
            center = pdata["Coordinates"][np.argmax(pdata["Density"])]
        else:
        ...
        if (idx + 1) % 50 == 0 or idx == 0:
            print(
                f"  snap {snap_num:04d}  t={t * 1e3:.2f} kyr"
                f"  M_enc_max={M_enc[-1]:.2f} Msun"
                f"  M_disk={M_disk:.3f} Msun"
                f"  n_sinks={n_sinks}"
                f"  [{idx + 1}/{len(snap_items)}]",
                flush=True,
            )
    ...
    # Store the smoothed, monotonic M_enc (NOT the raw per-snapshot array) so that
    # cumsum(M_shell) == M_enc exactly. Downstream ξ/Γ code needs M_tot(r) =
    # gas+sinks enclosed at r and Mdot from the same M_shell time series; a single
    # internally consistent M_enc avoids raw-vs-smoothed mismatches.
    M_enc_arr = M_enc_sm
